...
(use-package! aidermacs
  ...
  (defun aidermacs-get-api-key (key-name)
    "Get API key from auth-source or 1Password."
    (or (ignore-errors
         (auth-source-pick-first-password :host key-name))
        (string-trim (shell-command-to-string
                     (format "op item get '%s' --fields 'credential' --reveal" key-name)))))

  (defun set-aidermacs-env-vars ()
    "Set environment variables for Aidermacs."
    (let ((dashscope-key (aidermacs-get-api-key "DASHSCOPE_API_KEY"))
          (deepseek-key (aidermacs-get-api-key "DEEPSEEK_API_KEY")))
      (unless (and dashscope-key deepseek-key)
        (user-error "Failed to retrieve API keys"))
      (setenv "OPENAI_BASE_URL" "https://dashscope-intl.aliyuncs.com/compatible-mode/v1")
      (setenv "OPENAI_API_KEY" dashscope-key)
      (setenv "DEEPSEEK_API_KEY" deepseek-key)))

  (add-hook 'aidermacs-before-run-backend-hook #'set-aidermacs-env-vars)
